...
from instanceseg.datasets.runtime_transformations import \
    SemanticAgreementForInstanceLabelsRuntimeDatasetTransformer
...
class DataloaderDataIntegrityChecker(object):
        ...
        for tv in self.thing_values:  # Things must have instance id != 0
            ...
            if sem_locs.sum() > 0:
                if self.instance_problem.map_to_semantic:
                    # I think we need thing value 1 (things we're actually predicting) and thing
                    # value 0 (things we're
                    # not bothering to predict)
                    assert (inst_lbl[sem_locs] == 1).sum() + (
                            inst_lbl[sem_locs] == 0).sum() == sem_locs.sum()
                else:
                ...
                # assert inst_lbl[sem_locs].max() <=
                # self.instance_problem.n_instances_by_semantic_id[semantic_idx]

    def stuff_constraint(self, sem_lbl, inst_lbl):
        for sv in self.stuff_values:
            assert (inst_lbl[
                        sem_lbl == sv] != 0).sum() == 0, 'Stuff should only have instance label ' \
                                                         '0, ' \
                                                         'not {}'.format(
                inst_lbl[sem_lbl == sv].max())
    ...
    def sem_value_constraint(self, sem_lbl):
        ...
        for semval in semvals:
            assert semval in self.instance_problem.semantic_transformed_label_ids or semval == \
                   self.void_val, \
                'Semantic label contains value {} which is not in the following list: {}'.format(
                    semval, '\n'.join(['{}: {}'.format(v, n)
                                       for v, n in
                                       zip(self.instance_problem.semantic_transformed_label_ids,
...
def transform_and_export_input_images(trainer: Trainer, dataloader,
    ...
    if write_transformed_images:
        transformed_image_export(dataloader, max_image_dim, n_debug_images, out_dir_parent, split,
                                 trainer)
...
def raw_image_exporter(dataloader, n_debug_images, out_dir_parent):
    ...
    try:
        ...
        raise Exception(
            'Dataset isnt in a format where I can retrieve the raw image files easily to copy them '
    ...
    t = tqdm.tqdm(enumerate(dataloader.sampler.indices),
                  total=len(dataloader) if n_debug_images is None else n_debug_images, ncols=120,
                  leave=False)
    ...
    for image_idx, idx_into_dataset in t:
        ...
        for filetype, filename in filename_d.items():
            ...
            if filetype is not 'inst_lbl':
                ...
                new_inst_lbl_file = os.path.join(out_dir_raw_rgb, filetype,
                                                 'modified_modep_' + out_name)
        ...
        decomposed_image_path = os.path.join(out_dir_raw_decomposed,
                                             'decomposed_%012d' % image_idx + imname + '.png')
...
def transformed_image_export(dataloader, max_image_dim, n_debug_images, out_dir_parent, split,
                             trainer):
    t = tqdm.tqdm(enumerate(dataloader),
                  total=len(dataloader) if n_debug_images is None else n_debug_images,
    ...
    for batch_idx, (img_data_b, (sem_lbl_b, inst_lbl_b)) in t:
        ...
        for datapoint_idx in range(img_data_b.size(0)):
            ...
            segmentation_viz = \
                trainer_exporter.visualization_utils.visualize_segmentations_as_rgb_imgs(
            ...
            out_img = create_instancewise_decomposed_label(sem_lbl, inst_lbl,
                                                           ...
                                                           cmap_dict_by_sem_val=cmap_dict_by_sem_val,
                                                           max_image_dim=max_image_dim)
            decomposed_image_path = os.path.join(out_dir_decomposed,
                                                 'decomposed_%012d.png' % image_idx)
            save_and_possibly_resize(decomposed_image_path, out_img)
...
def decompose_into_mask_image_per_instance(sem_lbl, inst_lbl, to_np=True):
    ...
    for sem_val in sem_vals:
        ...
        for inst_val in inst_vals_by_sem_val[sem_val]:
            ...
            instance_masks[sem_val].append(
                mask.numpy() if to_np and torch.is_tensor(mask) else mask)
...
def visualize_masks_by_sem_cls(instance_mask_dict, inst_vals_dict, cmap_dict_by_sem_val=None,
    ...
    assert all([len(lst1) == len(lst2) for lst1, lst2 in
                zip(instance_mask_dict.values(), inst_vals_dict.values())])
    ...
    if sem_names_dict is None:
        sem_names_dict = {sem_val: '{},'.format(
            sem_val if sem_val not in void_vals else '{} (void),'.format(sem_val))
            for sem_val in sem_vals}
    ...
    for sem_val in sem_vals:
        ...
        for inst_val, inst_mask in zip(inst_vals_dict[sem_val], instance_mask_dict[sem_val]):
            ...
            if use_funky_void_pixels and sem_val in void_vals:
                ...
                viz_void = (
                        np.random.random(
                            (void_inst_mask.shape[0], void_inst_mask.shape[1], 3)) * 255
                ).astype(np.uint8)
        ...
        true_label_mask_row = visualization_utils.get_tile_image_1d(true_label_masks,
                                                                    concat_direction='horizontal',
                                                                    margin_color=margin_color,
                                                                    margin_size=margin_size_small)
        colormap_row = visualization_utils.get_tile_image_1d(colormaps,
                                                             concat_direction='horizontal',
                                                             margin_color=margin_color,
                                                             margin_size=margin_size_small)
        row_with_colormaps = visualization_utils.get_tile_image_1d(
            [colormap_row, true_label_mask_row],
    ...
    sem_cls_rows = [
        visualization_utils.pad_image_to_right_and_bottom(sem_cls_row, dst_width=max_width)
...
def create_instancewise_decomposed_label(sem_lbl, inst_lbl, input_image=None, sem_names_dict=None,
    ...
    n_images_wide = max([len(ivs) for sv, ivs in inst_vals_dict.items()]) + (
        1 if input_image is not None else 0)
    ...
    downsample_multiplier = max(1, get_multiplier((out_img_height, out_img_width),
                                                  max_image_dim=max_image_dim))

    out_img = visualize_masks_by_sem_cls(instance_mask_dict, inst_vals_dict,
                                         cmap_dict_by_sem_val=cmap_dict_by_sem_val,
