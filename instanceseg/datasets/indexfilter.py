...
class ValidIndexFilter(object):
    ...
    def valid_indices(self):
        if self._valid_indices is None:
            ...
            self._valid_indices = self.get_valid_indices(sem_cls_filter, n_instance_ranges,
                                                         n_images)
    ...
    def get_valid_indices(self, sem_cls_filter, n_instance_ranges, n_images):
        if sem_cls_filter is None and n_instance_ranges is None and n_images is None:
            ...
            valid_indices = self.valid_indices_from_instance_stats_config(sem_cls_filter,
                                                                          n_instance_ranges,
                                                                          union=False)
    ...
    def valid_indices_from_instance_stats_config(self, semantic_class_vals, n_instance_ranges,
        ...
        if n_instance_ranges is None and semantic_class_vals is None:
            ...
            for sem_cls, n_instances_range in zip(semantic_class_vals, n_instance_ranges):
                valid_indices_single_set = self.get_valid_indices_single_sem_cls(
    ...
    def filter_images_by_instance_range_from_counts(instance_counts, n_instances_range=None,
        ...
        assert len(n_instances_range) == 2, ValueError(
        ...
        has_at_least_n_instances = (instance_counts >= n_instances_min) \
            if n_instances_min is not None else torch.ByteTensor(instance_counts.size()).fill_(1)
        ...
        if semantic_classes is None:
            valid_indices_as_tensor = has_at_least_n_instances.sum(
                dim=1) * has_at_most_n_instances.sum(dim=1)
        else:
            valid_indices_as_tensor = sum(
                [has_at_least_n_instances[:, sem_cls] for sem_cls in semantic_classes]) * \
                                      sum([has_at_most_n_instances[:, sem_cls] for sem_cls in
                                           semantic_classes])
