...
def get_non_symmetric_upsampling_weight(in_channels, out_channels, kernel_size, model_channel_semantic_ids=None):
    """
    Make a 2D bilinear kernel suitable for upsampling
    model_channel_semantic_ids: gives us a list of the classes to expand (from semantic to instance)
    """
    factor = (kernel_size + 1) // 2
    if kernel_size % 2 == 1:
        center = factor - 1
    ...
    if model_channel_semantic_ids is None:
        ...
        if len(model_channel_semantic_ids) != out_channels:
            if len(model_channel_semantic_ids) == in_channels:
                ...
                raise ValueError('I don''t know how to handle initializing bilinear interpolation weights from {} '
                                 'channels to {} channels without model_channel_semantic_ids being {} long'.format(
                                  in_channels, out_channels, out_channels))
        weight = np.zeros((in_channels, out_channels, kernel_size, kernel_size),
                          dtype=np.float64)
        # NOTE(allie): this for loop could be made more efficient if it becomes a bottleneck.
        for inst_cls_idx, sem_cls in enumerate(model_channel_semantic_ids):
