            ...
            if name == prefix:
                for n, p in module.named_parameters():
                    ...
                    for suffix in suffixes:
                        if (n.split('.')[-1].startswith(suffix) or n.endswith(
                                suffix)) and p.requires_grad:
                            yield p
                break
...
class DCNBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, fix_bn=True,
        ...
        self.conv2_offset = nn.Conv2d(planes, 18 * deformable_group, kernel_size=3, stride=1,
...
class res_block(nn.Module):
    def __init__(self, planes, blocks, block=Bottleneck, stride=1, dilation=1, fix_bn=True,
                 with_dpyramid=False):
...
class resnet_rcnn(nn.Module):
    ...
    def load_state_dict(self, state_dict, resume=False):
        ...
        if 'rcnn.cls_score.weight' in state_dict and own_state['rcnn.cls_score.weight'].shape[
            0] == 9 and state_dict['rcnn.cls_score.weight'].shape[0] == 81:
            cls_map = {
                0: 0,  # background
                1: 1,  # person
                2: -1,  # rider, ignore
            ...
            for weight_name in ['rcnn.cls_score.weight', 'rcnn.cls_score.bias',
                                'rcnn.bbox_pred.weight', 'rcnn.bbox_pred.bias',
                                'mask_branch.mask_score.weight', 'mask_branch.mask_score.bias']:
                mean = state_dict[weight_name].mean().item()
                std = state_dict[weight_name].std().item()
                state_dict[weight_name] = state_dict[weight_name].view(
                    *([81, -1] + list(state_dict[weight_name].shape[1:])))
                weight_blobs = ((np.random.randn(
                    *([9] + list(state_dict[weight_name].shape[1:])))) * std + mean).astype(
                    np.float32)
        ...
        if 'fcn_head.score.weight' in own_state and 'fcn_head.score.weight' in state_dict and \
                own_state['fcn_head.score.weight'].shape[0] == 19 and \
                state_dict['fcn_head.score.weight'].shape[0] == 133:
            cls_map = {
                0: 20,  # road
            ...
            for weight_name in ['fcn_head.score.weight', 'fcn_head.score.bias']:
                ...
                std = state_dict[weight_name].std().item()
                weight_blobs = ((np.random.randn(
                    *([19] + list(state_dict[weight_name].shape[1:])))) * std + mean).astype(
                    np.float32)
        ...
        for name, param in state_dict.items():
            ...
            if own_state[name].shape == param.shape:
                ...
                warnings.warn(
                    'While copying the parameter named {}, whose dimensions in the models are'
                    ' {} and whose dimensions in the checkpoint are {}, ...'.format(
                        name, own_state[name].size(), param.size()))

        missing = set(own_state.keys()) - set(
            [self.name_mapping(_, resume) for _ in state_dict.keys()])
        if len(missing) > 0:
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, blocks):
        ...
        self.res3 = res_block(128, blocks[1],
                              block=DCNBottleneck if self.with_dconv <= 3 else Bottleneck,
                              stride=2, fix_bn=self.fix_bn, with_dpyramid=self.with_dpyramid)
        self.res4 = res_block(256, blocks[2],
                              block=DCNBottleneck if self.with_dconv <= 4 else Bottleneck,
                              stride=2, fix_bn=self.fix_bn, with_dpyramid=self.with_dpyramid)
        if self.with_dilation:
            res5_stride, res5_dilation = 1, 2
        else:
            res5_stride, res5_dilation = 2, 1
        self.res5 = res_block(512, blocks[3],
                              block=DCNBottleneck if self.with_dconv <= 5 else Bottleneck,
    ...
    def forward(self, x):
        ...
        res_conv1 = self.conv1(x).detach() if self.freeze_at == 1 else self.conv1(x)
        res2 = self.res2(conv1).detach() if self.freeze_at == 2 else self.res2(res_conv1)
