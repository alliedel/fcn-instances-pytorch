...
def main():
    ...
    cfg, cfg_to_print = get_cfgs(dataset_name=args.dataset, config_idx=config_idx,
                                 cfg_override_args=cfg_override_args)
    cfg['sampler'] = args.sampler
    assert cfg[
               'dataset'] == args.dataset, 'Debug Error: cfg[\'dataset\']: {}, args.dataset: {}'.format(
        cfg['dataset'],
        args.dataset)
    ...
    out_dir = instanceseg.utils.logs.get_log_dir(osp.basename(__file__).replace('.py', ''),
                                                 config_idx,
                                                 cfg_to_print,
                                                 parent_directory=os.path.join(here, 'logs',
                                                                               args.dataset))
    ...
    dataloaders = instanceseg.factory.data.get_dataloaders(cfg, args.dataset, args.cuda,
                                                           sampler_cfg)
    ...
    problem_config = instanceseg.factory.models.get_problem_config(
        dataloaders['val'].dataset.semantic_class_names, n_instances_per_class,
    ...
    print(
        'Loaded example images: img_ss, (sem_lbl_ss, inst_lbl_ss) as sampled version and img, (sem_lbl, '
