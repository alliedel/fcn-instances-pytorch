...
def panoptic_converter(original_format_folder, out_folder, out_file):
    ...
    for working_idx, f in enumerate(file_list):
        ...
        file_name, image_json_entry, json_annotation_entry, pan_format = generate_coco_annotation_format_for_image(
            categories_dict, f)

        annotations.append(json_annotation_entry)
        Image.fromarray(pan_format).save(os.path.join(out_folder, file_name))
        images.append(image_json_entry)
...
def generate_coco_annotation_format_for_image(categories_dict, f):
    ...
    for el in l:
        ...
        segm_info.append({"id": int(segment_id),
                          ...
                          "bbox": bbox,
                          "iscrowd": is_crowd})
    json_annotation_entry = {'image_id': image_id,
                             'file_name': file_name,
                             'segments_info': segm_info}
    image_json_entry = {"id": image_id,
                        "width": original_format.shape[1],
                        "height": original_format.shape[0],
                        "file_name": image_filename}
    return file_name, image_json_entry, json_annotation_entry, pan_format


if __name__ == "__main__":
    split = 'val'
    converted_cityscapes_root = os.path.expanduser('~/data/datasets/cityscapes_data')
    original_format_folder = os.path.expanduser('~/data/datasets/cityscapes/gtFine/{}/'.format(split))
    # folder to store panoptic PNGs
    out_folder = os.path.join(converted_cityscapes_root, 'cityscapes_panoptic_{}/'.format(split))
    # json with segmentations information
    out_file = os.path.join(converted_cityscapes_root, 'cityscapes_panoptic_{}.json'.format(split))
