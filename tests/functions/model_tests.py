...
def build_example_model(**model_cfg_override_kwargs):
    ...
    for k, v in model_cfg_override_kwargs.items():
        cfg[k] = v
    problem_config = torchfcn.factory.models.get_problem_config(ALL_VOC_CLASS_NAMES, 2, map_to_semantic=cfg['map_to_semantic'])
...
def test_forward_hook():
    ...
    dataloaders = torchfcn.factory.data.get_dataloaders(cfg, 'voc', cuda, sampler_cfg=None)
